	...
	RansacCurvature<PointSource, PointTarget, Scalar>::align_redux( PointCloudSource &output, const Eigen::Matrix4f& guess )
{
	...
	if (this->indices_->size () != input_->points.size ())
	{
	...
	converged_ = false;
	final_transformation_ = this->transformation_ = this->previous_transformation_ = Eigen::Matrix4f::Identity ();
